...
欢迎来到MLlab2025！本Lab涵盖了线性回归（10 pts）、逻辑回归（10 pts）、KNN（10 pts）、Decision Tree（10 pts）、Adaboost（10 pts）、SVM（20 pts）、Neural Network（30 pts）等机器学习算法的基本实践。在 Neural Network Lab 中，我们额外提供了一个可供选做的附加题，共有 10 个 Bonus pts。
...
我们在大部分 Lab 中提供了自动评分器 autograder.py，用于对代码进行评分。autograder.py 会通过单元测试对你编写的代码进行自动化评估。你无需单独运行该文件；在完成各子 Lab 的 .ipynb 文件时，我们会指示你在特定步骤调用 autograder.py 中的某些函数进行分数评估。

**注意事项：**

1. **认真阅读函数注释**：请仔细阅读每个Lab中函数的注释，确保你返回的变量完全符合要求，以顺利通过`autograder.py`的评分。

2. **关于测试样例**：我们提供给你的单元测试样例可能并不全面。最终评分时，我们可能使用其他测试样例进行验证。因此，请避免仅针对提供的样例进行“过拟合”优化，务必确保代码的通用性和正确性。
3. **自动测试异常**：`autograder.py`可能并不完全鲁棒。在某些情况下，即使代码正确，评分器可能仍会抛出错误。如果你确信你的代码无误，请随时联系课程助教以解决问题。
